...
const loadDashBoard = async (req, res) => {
    try {
      ...
      const pendingOrdersRequest = await orderModel.find({ "products": { $elemMatch: { orderStatus: "Requested Return" } } }).countDocuments();
      const totalProductCancelled = await orderModel.find({ "products": { $elemMatch: { orderStatus: "Canceled" } } }).countDocuments();
    const totalReturnedProducts = await orderModel.aggregate([
      ...
      const placed = await orderModel.find({ "products": { $elemMatch: { orderStatus: "Placed" } } }).countDocuments();
      const shipped = await orderModel.find({ "products": { $elemMatch: { orderStatus: "Shipped" } } }).countDocuments();
      const packed = await orderModel.find({ "products": { $elemMatch: { orderStatus: "Packed" } } }).countDocuments();
      const delivered = await orderModel.find({ "products": { $elemMatch: { orderStatus: "Delivered" } } }).countDocuments();
